  ...
  try {
    ...
    if (allEvents.length === 0) {
      return NextResponse.json(
        { success: false, message: "No events found in database" },
        { status: 404 }
      );
    }
    ...
    // Update each event that doesn't have a description or has a very short one
    for (const event of allEvents) {
      const hasDescription = event.description && event.description.trim().length >= minLength;
      const shouldUpdate = force || !hasDescription;
      ...
      if (shouldUpdate) {
        ...
        results.push({
          ...
          title: event.title,
          status: 'skipped',
          reason: `Already has description (${event.description?.trim().length || 0} chars)`,
        });
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Updated ${updatedCount} event(s) with dummy content`,
      totalEvents: allEvents.length,
      updated: updatedCount,
      skipped: allEvents.length - updatedCount,
      results,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      {
        success: false,
        message: 'Failed to populate event content',
        error: error instanceof Error ? error.message : 'Unknown error',
      },
      { status: 500 }
    );
